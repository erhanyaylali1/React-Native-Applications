...
import Events from './events'; // @flow
...
export var lockBodyScroll = function lockBodyScroll(targetElement, options) {
  if (isIosDevice) {
    ...
    if (targetElement && !locks.some(function (lock) {
      ...
      if (!documentListenerAdded) {
        Events.on(document, 'touchmove', preventDefault); // document.addEventListener('touchmove', preventDefault, hasPassiveEvents ? { passive: false } : undefined);
...
export var clearAllBodyScrollLocks = function clearAllBodyScrollLocks() {
  if (isIosDevice) {
    ...
    if (documentListenerAdded) {
      Events.off(document, 'touchmove', preventDefault); // document.removeEventListener('touchmove', preventDefault, hasPassiveEvents ? { passive: false } : undefined);
...
export var unlockBodyScroll = function unlockBodyScroll(targetElement) {
  if (isIosDevice) {
    if (!targetElement) {
